...
(async () => {
  const { owner, repo, token, removeAll } = await inquirer.prompt([
    ...
    },
    {
      ...
      default: false,
      message:
        "Do you want to remove all deployments, including successful ones?",
    },
  ]);
  ...
  try {
    ...
    const deploymentsList = await makeRequest("/deployments").then((res) =>
      res.json()
    );
    ...
    for await (const id of ids) {
      ...
      const statusesGetResponse = await makeRequest(
      ...
      if (state === "success") {
        ...
        await makeRequest(`/deployments/${id}/statuses`, {
          method: "POST",
      ...
      await makeRequest(`/deployments/${id}`, { method: "DELETE" }).then(
        (res) => res.text()
      );
    }
